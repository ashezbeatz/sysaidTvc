...
class PostData {
    ...
    static async saveDetails(tiketid, sr_title, requestor, responsibility, assign_group, company, status, sr_type, ticket_date, input_type, service_name, approved_test_cases, test_case_approver, cleared, request_clearance_cab, cab_cleared_by, test_type, performance_received, performance, results, breach, test_lead, timeline, comments) {
        let connection;
        try {
            connection = await db.pool.getConnection();
